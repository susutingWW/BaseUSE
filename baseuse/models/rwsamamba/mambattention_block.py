# Vendored from https://github.com/NikolaiKyhne/RWSAMamba-UNet (models/mambattention_block.py)
# No functional change; a duplicated `self.attention` assignment in
# MambAttentionBlock.__init__ was removed. TFMambaBlock (attention-free) is
# the block used when the generator is configured with use_rwsa: false.
...
class MambaBlock(nn.Module):
    def __init__(self, in_channels, cfg):
        ...
        n_layer = 1
        self.fp32_island = bool(cfg['model_cfg'].get('mamba_fp32', False))
    ...
    def forward(self, x):
        # mamba_fp32: the selective-scan backward accumulates state products
        # that lose precision / overflow in bf16 and are a known NaN source;
        # run the scan in fp32 inside the surrounding bf16 autocast region.
        if self.fp32_island and torch.is_autocast_enabled():
            with torch.autocast(device_type=x.device.type, enabled=False):
                return self._scan(x.float())
        return self._scan(x)
...
class TFMambaBlock(nn.Module):
    """
    Temporal-Frequency Mamba block (no attention) for sequence modeling.
...
class MambAttentionBlock(nn.Module):
    ...
    Only instantiated when the generator is configured with use_rwsa: true.
    ...
    flinear (ConvTranspose1d): ConvTranspose1d layer for frequency dimension.
    """
    def __init__(self, cfg, inchannels, shared_attention=None):
        ...
        # Use shared attention if provided
        self.attention = shared_attention if shared_attention is not None else AttentionModule(dim=self.hid_feature, n_head=8)
        self.tlinear = nn.ConvTranspose1d(self.hid_feature * 2, self.hid_feature, 1, stride=1)
        self.flinear = nn.ConvTranspose1d(self.hid_feature * 2, self.hid_feature, 1, stride=1)

# Vendored from https://github.com/NikolaiKyhne/RWSAMamba-UNet (models/codec_module.py)
# Modification (SFI multi-sampling-rate): MagDecoder's LearnableSigmoid2D is
# sized from cfg['stft_cfg']['freq_bins_max'] (a frequency table for the max
# sampling rate, sliced at runtime) instead of a fixed n_fft//2+1.

# Modification (NaN guard): PhaseDecoder's atan2 gets +EPS on both inputs.
# The backward of atan2(0, 0) is 0/0 = NaN; under bf16 autocast exactly-zero
# conv outputs (silence / padding regions) are common enough to poison ~30%
# of training steps. Mirrors the addeps convention in rwsa_se.analyze.
EPS = 1e-8
...
def get_padding_2d(kernel_size, dilation=(1, 1)):
    ...
    - dilation (tuple, optional): Dilation rate of the convolutional layer (height, width). Defaults to (1, 1).
...
class MagDecoder(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.freq_bins_max = cfg['stft_cfg']['freq_bins_max']
        ...
        self.lsigmoid = LearnableSigmoid2D(self.freq_bins_max, beta=self.beta)

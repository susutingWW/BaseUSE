...
class BSRNN(nn.Module):
    ...
    def __init__(
        ...
        # recompute layer activations in backward instead of storing them all
        # (~6x activation memory saving, ~30% slower steps); needed for long
        # low-fs batches (e.g. 96000 samples @ 8kHz = 12s = 1500 STFT frames)
        self.gradient_checkpointing = gradient_checkpointing

    def _bsrnn_layer(self, skip, t, i):
        """One (time-RNN + freq-RNN) dual block. Returns the updated skip."""
        B, N, T, K = skip.shape
        ...
        for i in range(self.num_layer):
            if self.gradient_checkpointing and self.training and torch.is_grad_enabled():
                skip = torch.utils.checkpoint.checkpoint(
                    self._bsrnn_layer, skip, t, i, use_reentrant=False
                )
            else:
                skip = self._bsrnn_layer(skip, t, i)

...
################################################################
def _resolve_local_checkpoint(repo_id):
    """Resolve a HF repo id to its cached snapshot dir when available.

    transformers 4.41's from_pretrained issues a network `has_file` HEAD check
    for repo ids even when the model is fully cached and offline env vars are
    set; on machines without internet this times out. Passing the local
    snapshot directory instead makes from_pretrained fully offline.
    """
    try:

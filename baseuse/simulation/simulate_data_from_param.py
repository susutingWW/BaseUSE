...
from baseuse.simulation.generate_data_param import get_parser
from baseuse.simulation.rir_utils import estimate_early_rir
...
# ffmpeg binary used for wind-noise simulation; override with FFMPEG_PATH
...
def process_one_sample(
    ...
    if info["noise_uid"].startswith("wind_noise"):
        ...
        match = re.fullmatch(
            rf"wind_noise\(threshold=(.*),ratio=(.*),attack=(.*),release=(.*),sc_gain=(.*),clipping=(.*),clipping_threshold=(.*)\)",
            augmentation[0],
        )
        (
            threshold_,
    ...
    for augmentation in augmentations:
        if augmentation == "none" or augmentation == "":
            ...
            match = re.fullmatch(rf"bandwidth_limitation-(.*)->(\d+)", augmentation)
            ...
            match = re.fullmatch(rf"clipping\(min=(.*),max=(.*)\)", augmentation)
            ...
            match = re.fullmatch(
                rf"codec\(format=(.*),encoder=(.*),qscale=(.*)\)", augmentation
            )
            ...
            match = re.fullmatch(
                rf"packet_loss\(packet_loss_indices=(.*),packet_duration_ms=(.*)\)",
                augmentation,
            )
